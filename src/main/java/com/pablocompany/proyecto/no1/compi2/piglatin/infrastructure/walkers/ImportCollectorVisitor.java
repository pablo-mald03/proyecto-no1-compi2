...
/**
 * Collects all ImportNodePigLatin found in a .pig AST.
 * Returns a list of semantic ImportNode models, decoupled from the AST.
 */
@Getter
public class ImportCollectorVisitor implements PigLatinAstVisitor<Void> {


    private final List<ImportNode> imports;
    private final String declaredIn;
    private final String declaredInName;

    public ImportCollectorVisitor(String declaredIn, String declaredInName) {
        this.imports = new ArrayList<>();
        this.declaredIn = declaredIn;
        this.declaredInName = declaredInName;
    }


    /**
     * Program node delegation
     *
     */
    @Override
    public Void visit(ProgramNodePigLatin node) {
        if (node.getBody() != null) {
            for (ImportNodePigLatin child : node.getBody().getImports()) {
                if (child != null) {
                    child.accept(this);
                }
            }
    ...
    public Void visit(ImportNodePigLatin node) {
        ...
        importNode.setRawPath(node.getImportPath());
        importNode.setDeclaredIn(declaredIn);
        importNode.setDeclaredInName(declaredInName);
        importNode.setLine(node.getLine());
        importNode.setColumn(node.getColumn());
